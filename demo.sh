...
set -e 
echo Starting HDFS...
docker-compose up namenode datanode1 datanode2 

echo Starting HBase...
docker-compose up hbase_zookeeper hbase_master hbase_regionserver hbase_thrift hbase_rest

echo "Starting Lilly, Solr and Banana (ELK, lol)..."
docker-compose up lily solr banana
...
curl -iv -F template=@config/hue/users.xml -X POST $API/process-groups/$PROCESS_GROUP/templates/upload  
